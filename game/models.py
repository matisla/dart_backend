...
class Leg(models.Model):
    ...
    @property
    def winner(self):
        "return the user who win the leg or None"
        scores = {}

        for round in self.rounds.all():
            scores[round.user] = scores.get(round.user, self.set.game.max_score) - round.score

            if scores[round.user] == 0:
                return round.user
...
class Round(models.Model):
    ...
    def __str__(self):
        return 'no score' if not self.score else self.score
